layui.use(["layer", "laydate"], function () {
    ...
    $("#memu1 input:eq(0), #TitleMemu input:eq(0)").on("click", function () {
        ...
        if (Ajax("/cctv/pipe/append", {id: id2, no: index}))
            window.location.href = "checkview?id=" + id2 + "&no=" + length;
    });
    /** 保存数据按钮 */
    ...
    $("#tab1 tbody tr").each(function (no) {
        ...
        $(this).find("input:eq(1)").click(function () {
            layer.confirm(tipsText2, {
                ...
                if (Ajax("/cctv/pipe/remove", {id: id}))
    ...
    $("#tab2").on("input", "tr td:nth-child(6) input", function () {
        ...
        if ($("#tab2 tbody tr").eq(itemindex).find("td:eq(11) input").val() !== "")
            return false;
        let dist = $(this).parents("tr").find("td:eq(3) input").val();
    ...
    $("#log").on("click", function () {
        if ($("#tab3 tbody tr").length === 0) {
            let context = "";
            const data = Ajax("/cctv/note/findlist", {id: id1});
            for (let i = 0; data != null && i < data.length; i++) {
                context += "<tr>";
                context += "  <td width='115px'>[" + data[i].date + "]</td>";
                context += "  <td width='660px' align='left'>" + data[i].context + "</td>";
