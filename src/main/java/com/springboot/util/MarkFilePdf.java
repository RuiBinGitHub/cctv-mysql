...
@Component
public class MarkFilePdf extends PdfPageEventHelper {
    ...
    private final Paragraph wrap = new Paragraph("\n"); // 换行符
    ...
    public void initPDF(Project project, String path) {
        ...
        File file = new File(FileName + "_CCTV.pdf");
        ...
        Map<String, List<Pipe>> pipeMap = pipes.stream().peek(pipe -> {
            ...
            if (pipe.getVideono() == null)
                pipe.setVideono("");
        }).collect(Collectors.groupingBy(pipe -> pipe.getWorkorder() + pipe.getUses()));
        List<String> keys = pipeMap.keySet().stream().sorted(String::compareTo).collect(Collectors.toList());
        ...
        for (Pipe pipe : pipes) {
            ...
            while (items != null && items.size() > index) {
                ...
                int z = Math.min(index + 30, items.size());
                double size1 = AppUtils.getDouble(items.get(index).getDist());
                double size2 = AppUtils.getDouble(items.get(z - 1).getDist());
                ...
                graphics.setFont(new java.awt.Font("", java.awt.Font.PLAIN, 9));
                ...
                graphics.setFont(new java.awt.Font("", java.awt.Font.PLAIN, 6)); // 设置画笔
                while (j < z) {
                    Item item = items.get(j);
                    distance = (int) ((AppUtils.getDouble(item.getDist()) - size1) / pipesize * length) + 50;
                    ...
                    if (item.getGrade() >= 4) {
                        graphics.setFont(new java.awt.Font("Helvetica", java.awt.Font.BOLD, 9));
                        ...
                        graphics.setFont(new java.awt.Font("Helvetica", java.awt.Font.PLAIN, 9));
                    ...
                    if (item.getCode().length() > 2 && item.getCode().contains("-"))
                    ...
                    graphics.drawString(foramt3.format(AppUtils.getDouble(item.getDist())), 150, location + 3);
            ...
            for (int k = 0; items != null && k < items.size(); k++) {
                ...
                // 添加图片
                image = Image.getInstance(myfile + "/image/" + item.getPicture() + ".png");
    ...
    private static Font getChineseFont() {
        try {
            ...
            Font font = new Font(baseFont, 8, 0);
    ...
    public void getWidths(int[] widths, int index, int... value) {
        System.arraycopy(value, 0, widths, index, value.length);
    }
    ...
    private PdfPTable getTableFormA(Pipe pipe) {
        Company company = pipe.getProject().getUser().getCompany();
        map = AppUtils.getMap("name", pipe.getOperator(), "company", company);
        ...
        table.addCell(getCell(pipe.getProject().getName(), font2, 1, 1, 1, 0, 0));
        ...
        table.addCell(getCell(pipe.getProject().getClient(), font2, 4, 1, 1, 0, 0));
...
//		table.addCell(getCell(pipe.getTime(), font2, 1, 1, 1, 0, 0));
    ...
    private PdfPTable getTableFormC(Pipe pipe) {
        ...
        appendTableValue(pipeInfo, pipe.getProject().getName(), font2, 10, 1, border);
    ...
    private PdfPTable getItemFormD(Pipe pipe, Item item) {
        ...
        String name = pipe.getProject().getName() + "/" + pipe.getWorkorder() + "/" + pipe.getNo();
    ...
    private List<String> getTextList(String text, PdfGraphics2D graphics) {
        ...
        for (int i = 0; i < text.length(); i++) {
            context.append(text.charAt(i));
            int length = fontMetrics.stringWidth(context.toString());
